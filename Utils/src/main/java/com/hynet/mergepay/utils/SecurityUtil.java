...
//        KeyGenerator.getInstance(Regex.AES.getRegext()).init(128, new SecureRandom(key.getBytes()));
//        Cipher cipher = Cipher.getInstance(Regex.AES.getRegext());
//        cipher.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(KeyGenerator.getInstance(Regex.AES.getRegext()).generateKey().getEncoded(), Regex.AES.getRegext()));
...
//        KeyGenerator.getInstance(Regex.AES.getRegext()).init(128, new SecureRandom(key.getBytes()));
//        Cipher cipher = Cipher.getInstance(Regex.AES.getRegext());
//        cipher.init(Cipher.DECRYPT_MODE, new SecretKeySpec(KeyGenerator.getInstance(Regex.AES.getRegext()).generateKey().getEncoded(), Regex.AES.getRegext()));
...
//    }
    ...
    public byte[] encryptAES(String data, String key) throws InvalidKeyException, NoSuchPaddingException, NoSuchAlgorithmException, UnsupportedEncodingException, BadPaddingException, IllegalBlockSizeException {
        LogUtil.getInstance().print(data);
        LogUtil.getInstance().print(key);
        Cipher cipher = Cipher.getInstance(Regex.AES.getRegext());
