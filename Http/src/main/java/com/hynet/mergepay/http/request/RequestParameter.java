...
import com.hynet.mergepay.http.listener.OnHttpRequestTaskListener;
...
//            body = RequestBody.create(MediaType.parse(Regex.DEFAULT_TYPE.getRegext()), json);
            ...
            for (Parameter parameter : mFiles) {
                String key = parameter.getKey();
                com.hynet.mergepay.http.model.File file = parameter.getFile();
                if (file != null) {
                    hasData = true;
...
//    }
    ...
    public void addFormDataParameter(String key, File file, String contentType) {
        if (isFileAvalable(file)) {
            ...
            addFormDataParameter(key, new com.hynet.mergepay.http.model.File(file, mediaType));
        }
    }
    ...
    public void addFormDataParameter(String key, com.hynet.mergepay.http.model.File file) {
    ...
    public void addFormDataParameter(String key, File file) {
        if (isFileAvalable(file)) {
            ...
            if (!isPng && !isJpg) {
                addFormDataParameter(key, new com.hynet.mergepay.http.model.File(file, null));
            }
        }
    }
    ...
    public void addFormDataParameter2(String key, List<com.hynet.mergepay.http.model.File> files) {
        for (com.hynet.mergepay.http.model.File file : files) {
            addFormDataParameter(key, file);
        }
    }
